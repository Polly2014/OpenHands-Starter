...
#         $pullRuntime = docker pull docker.all-hands.dev/all-hands-ai/runtime:0.27-nikolaik
...
function Pull-DockerImages {
    ...
    try {
        ...
        $runtimePullProcess = Start-Process -FilePath "docker" -ArgumentList "pull", "docker.all-hands.dev/all-hands-ai/runtime:0.27-nikolaik" -NoNewWindow -PassThru -Wait
        ...
        $runtimeImage = docker images --format "{{.Repository}}:{{.Tag}}" | Where-Object { $_ -eq "docker.all-hands.dev/all-hands-ai/runtime:0.27-nikolaik" }
